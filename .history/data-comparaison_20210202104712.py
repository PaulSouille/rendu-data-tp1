...
for index,df in enumerate(array_df):
    ...
    for index_, column in enumerate(df):
        canvas.create_text(400,10+(index_+1)*80,fill="black",font="Times 15 bold",
                            text="Mois : "+str(column))
        canvas.create_text(400,30+(index_+1)*80,fill="black",font="Times 10",
                                text="Moyenne : "+str(df[column].mean()))
        canvas.create_text(400,50+(index_+1)*80,fill="black",font="Times 10",
                text="Ecart type : "+str(df[column].std()))
        canvas.create_text(400,70+(index_+1)*80,fill="black",font="Times 10",
