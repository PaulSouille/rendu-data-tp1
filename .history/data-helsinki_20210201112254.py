...
df = pd.read_excel('helsinki.xlsx', skiprows = 2,  nrows= 32, usecols = 'C:O',sheet_name=0)
df = df.drop(df.index[0])
df.drop(df.columns[0], axis=1, inplace=True)

df_error = pd.read_excel('helsinki.xlsx', skiprows = 2,  nrows= 32, usecols = 'C:O',sheet_name=1)
...
class matplotlibSwitchGraphs:
    ...
    def draw_graph(self, month):
        ...
        if(month == 'année'):
            ...
            self.ax.clear()
            self.ax.plot(df_error[month],label='Savukoski kirkonkyla')
